...
- `streamAnalysis(files, apiKey, priority)` (`:775`) — async generator:
  ...
  - Skips `[DONE]` via `continue` (`:842`); parses each `DeepSeekStreamChunk`;
    accumulates `delta.content` (`:847`), logs `reasoning_content` for reasoner (`:848-853`).
...
- **Final chunk dropped**: the loop `break`s on `done` (`:834`) before flushing; `decoder.decode`
  is called with `{stream:true}` but `decoder.flush()` is never called (`:836`). A multi-byte
  character split across the last chunk boundary can be corrupted/lost.
